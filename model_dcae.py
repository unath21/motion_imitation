...
class DCAEConfig:
    ...
    essence_extractor: EssenceExtractorConfig = field(default_factory=EssenceExtractorConfig)
...
class Decoder(nn.Module):
    def __init__(self, cfg: DecoderConfig):
        ...
        self.project_in = build_decoder_project_in_block(
            ...
            out_channels=cfg.width_list[-2],
            shortcut=cfg.in_shortcut,
        )

        self.project_essence = build_decoder_project_in_block(
            in_channels=cfg.latent_channels,
            out_channels=cfg.width_list[-2],
            shortcut=cfg.in_shortcut,
        )
    ...
    def forward(self, z: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        x = self.project_in(x)
        z = self.project_essence(z)
        x = torch.cat([z, x], dim=1)
        x = x.reshape(x.shape[0], 2, x.shape[1] // 2, x.shape[2], x.shape[3])
...
class EssenceExtractor(nn.Module):
    def __init__(self, cfg: EssenceExtractorConfig):
        # latent_dim: b, c, h, w
        super().__init__()
        self.cfg = cfg
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        b, c, h, w = x.shape
        x = x.view(b, c, h * w)
        return self.extractor(x).view(b, c, h, w)
...
class DCAE(nn.Module):
    def __init__(self, cfg: DCAEConfig):
        ...
        self.extractor = EssenceExtractor(cfg.essence_extractor)
...
def dc_ae_f32c32(name: str, pretrained_path: str) -> DCAEConfig:
    if name in ["dc-ae-f32c32-in-1.0", "dc-ae-f32c32-mix-1.0"]:
        cfg_str = (
            ...
            "essence_extractor.hidden_dim=64"
        )
