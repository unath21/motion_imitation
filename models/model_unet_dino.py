...
def get_dino_features(x):
	bs, c, h, w = x.shape
	device = x.device

	dinov2_model.eval().to(device)
	n_patch = 518 // dinov2_model.patch_size
	# resize x to 518x518
	x = F.interpolate(x, size=(518, 518), mode='bilinear', align_corners=True).to(device)

	with torch.no_grad():
		features = dinov2_model(x, is_training=True)
	...
	patchtokens = torch.max(patchtokens, dim=1).values  # [B, 1024, H', W'] -> [B, H', W']
	patchtokens = F.interpolate(patchtokens.unsqueeze(1), size=(h, w), mode='bilinear', align_corners=True).to(device)
	return patchtokens


class AutoencoderDINO(nn.Module):
	def __init__(self, in_channels=9, out_channels=3, z_channels=128):
		super().__init__()
	...
	def forward(self, x1, x2, dino1, dino2):
		img_diff = x2 - x1
		img_diff = torch.cat([img_diff, dino1, dino2], dim=1)  # [B, 3+1+1, H, W]
