...
def generate_and_save_masks(video_id, data_root, model, processor, device, save_dir="/data/pturaga/datasets/Penn_Action"):
    ...
    point_coords_x = mat['x'][0]
    point_coords_y = mat['y'][0]

    point_coords = []
    for i in range(13):
        point_coords.append([int(point_coords_x[i]), int(point_coords_y[i])])

    point_coords = [[[list(x) for x in set(tuple(p) for p in point_coords)]]]
    labels = [[[1 for _ in range(len(point_coords[0][0]))]]]
